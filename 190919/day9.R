...
data <- read.csv("./data/Part-3/three_sample.csv",header = T)
head(data)
...
#해설 : 검정 통계량 p-value 값은 0.1905 > 0.05(유의수준)
#세 집단 간의 분포 형태가 동질하다고 볼 수 있다.

# 세 집단의 평균 차이 검정 : aov()
result <- aov(score ~ method2, data=data2)
names(result) #검정 결과 측정된 변수명 확인
summary(result)

#해석 : 검정 통계량 p-value 값은 9.39e-14으로 유의수준 0.05보다 매우
#작기 때문에 교육방법에 따른 세 집단 간의 실기시험 평균에 차이가 있다.
#(연구가설 채택)
#F검정 통계량 43.58은 -1.96~+1.96의 범위의 귀무가설의 채택역에 해당되지 않음 
#귀무가설을 기각할 수 있다(연구가설 채택)
...
#결론 : 교육방법에 따른 세 집단 간의 실기시험 평균에 차이가 있으며
#세 교육방법에 따른 분석 결과 방법2와 방법1의 차이가 가장 크다.



###################연습문제########################
#연습문제01> 중소기업에서 생산한 HDTV 판매율을 높이기 위해서 프로모션을 진행한 결과 기존 구
#매비율 보다 15% 향상되었는지를 각 단계별로 분석을 수행하여 검정하시오.
#연구가설(H1) : 기존 구매비율과 차이가 있다.
#귀무가설(H0) : 기존 구매비율과 차이가 없다.
#조건) 구매여부 변수 : buy (1: 구매하지 않음, 2: 구매)

hdtv <- read.csv("./data/Part-3/hdtv.csv", header=TRUE)
# 단계2: 빈도수와 비율 계산
summary(hdtv)
length(hdtv$buy) # 50개
...
###############################################################
#연습문제02> 우리나라 전체 중학교 2학년 여학생 평균 키가 148.5cm로 알려져 있는 상태에서 A중
#학교 2학년 전체 500명을 대상으로 10%인 50명을 표본으로 선정하여 표본평균신장을
#계산하고 모집단의 평균과 차이가 있는지를 각 단계별로 분석을 수행하여 검정하시오.
# 단계1: 데이터
data<-read.csv("./data/Part-3/student_height.csv", header=TRUE)
...
#단계3: 정규성 검정
shapiro.test(x)  
#해석 : 검정 통계량 p-value 값은 0.0001853 로 유의수준 0.05보다 작기 때문에
#x 객체의 데이터 분포는 정규분포를 따르지 않는다.
...
#단계4: 가설검정 - 양측검정
#x객체와 기존 모집단의 평균 148.5시간 비교
wilcox.test(x, mu=148.5) # p-value = 0.067
wilcox.test(x, mu=148.5, alter="two.side", conf.level=0.95)
#해석 : 검정 통계량 p-value 값은 0.1212 로 유의수준 0.05보다 크다
#귀무가설 : 차이가 없다  0.05보다 크면 채택, 0.05보다 작으면 기각
#연구가설 : 차이가 있다 
#국내 중2 평균 키와 a중2 평균 키에 차이가 없다.
#x의 평균은 149.4 (점추정)는 신뢰구간에 포함되고(148.2531 ~ 150.5469)
#x의 평균은 신뢰구간에 포함되므로 귀무가설 채택(평균치 내에있다.)
#(신뢰구간은 귀무가설의 채택역의 의미가 있으므로)


################################################################
#연습문제03> 대학에 진학한 남학생과 여학생을 대상으로 진학한 대학에 대해서 만족도에 차이가 있
...
#해석 : 검정 통계량 p-value 값은 0.2765로 유의수준 0.05보다 크기 때문에 두 성별간 만족도 차이가 없다 (귀무가설 채택)
#검정 통계량 X-squared로 가설 검정을 수행하면 df 1일때 기각역은 3.84이고 X-squared 1.1845가 더 작기 때문에 귀무가설 채택

########################################################################
#연습문제04> 교육방법에 따라 시험성적에 차이가 있는지 검정하시오. (두 집단 평균 차이 검정)
...
#베리맥스 요인 회전법을 적용하여 요인 분석수행 p-value는 0.0232로
#유의수준 0.05보다 적기 때문에 요인수로 부족하다.
