...
plot(air_ctree)
#온도에 가장 큰 영향을 미치는 첫 번째 영향변수는 Ozone
#두번째 영향변수는 wind
#오존량 37 이하이면서 바람의 양이 15.5 이상이면 평균온도는 63정도에 해당
#바람의 양이 15.5이하인 경우 평균 온도는 70이상으로 나타남
#태양광은 온도에 영향을 미치지 않는 것으로 분석됨 



##########iris 데이터셋으로 분류 분석 수행#############
...
#data <- data.frame(AdultUCI$'capital-gain', AdultUCI$'hours-per-week', AdultUCI$'education-num', AdultUCI$race, AdultUCI$age, AdultUCI$income)
names(AdultUCI)
set.seed(1234)
...
#꽃 종류 변수를 분류하는 가장 중요한 변수는 Petal.Length와 Petal.Width로 보인다.
...
#정확도 ((yes-yes) + (no-no))/nrow(weather)
(278+53)/(278+13+22+53)
