...
def tel_web():
    ...
    if text == '로또' :
        ...
        random_year = random.randint(0,len(year_arr)-1)
        random_month = random.randint(0,len(month_arr)-1)
        random_day = random.randint(0,len(day_arr)-1)
        text = year_arr[random_year] + month_arr[random_month] + day_arr[random_day]
        send_msg = requests.get(f'{base_url}/sendMessage?chat_id={chat_id}&text={text}').json()
    elif '/번역' in text :
